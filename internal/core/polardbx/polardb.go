...
func NewPolarDB(conf *conf.PolarDB, logger_ log.Logger) *gorm.DB {

	log_ := log.NewHelper(log.With(logger_, "x_module", "data/new-dbr"))
	...
	if err != nil {
		log_.Fatalf("failed dbr use to mysqlx: %v", err)
	}
	return db
}
